...
async function createOnlyLinkInBM(title, brand, article, errorAddress) {
  try {
    ...
    const string =
      stringArr.join(' ') + ' ' + brand.replace(' ', '-') + ' ' + article
    let step = 0
    let newString = ''
    ...
    writeLogs(
      errorAddress,
      `createOnlyLinkInBM ${article} ${brand}  ${new Date()} - ${error.message}`
    )
  }
}
    ...
    axios
      ...
      .then(response => {
        ...
        if (!finaldata) {
          ...
          if (
            img === 'https://cdn.bm.parts/photos/' ||
            img === 'https://cdn.bm.parts/photo/'
          ) {
            resolve('-')
          }
          resolve(img)
      ...
      .catch(error => {
        writeLogs(
          errorAddress,
          `getFirstImageForItemFromBM ${art} ${brand} ${new Date()}- ${
            error.message
          }`
        )
      })
...
async function searchImageInAllSupliers(art, brand, error_path) {
  try {
    ...
    writeLogs(
      error_path,
      `searchImageInAllSupliers  ${art} ${brand} ${new Date()} - ${
        error.message
      }`
    )
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    axios
      ...
      .catch(error => {
        writeLogs(
          errorAddress,
          `getFirstImageAndTitleForItemFromBM ${art} ${brand} ${new Date()}- ${
            error.message
          }`
        )
      })
...
async function getFirstImageForItemFromTM(art, brand, error_path) {
  try {
    ...
    writeLogs(
      error_path,
      `getFirstImageForItemFromTM  ${art} ${brand} ${new Date()} - ${
        error.message
      }`
    )
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    axios.request(options).then(response => {
      if (typeof response.data.data[0] == 'undefined') {
        ...
        const rightBrand = response.data.data.find(
          branding =>
            branding.brand
              .toUpperCase()
              .slice(0, 5)
    ...
    writeLogs(
      error_path,
      `getIdBrandTechnoMir  ${article} ${brand} ${new Date()} - ${
        error.message
      }`
    )
  })
...
async function getImageTechnoMir(article, brandId, error_path) {
  return new Promise((resolve, reject) => {
    ...
    axios.request(options1).then(response1 => {
      if (
        response1.data.data?.images === undefined ||
        response1.data.data?.images[0] === undefined
      ) {
        resolve('-')
      } else {
        resolve(response1.data.data.images[0].image)
      }
    })
  }).catch(error => {
    writeLogs(
      error_path,
      `getImageTechnoMir  ${article} ${brandId} ${new Date()} - ${
        error.message
      }`
    )
  })
...
async function updateAllEndedProducts(suplierId, finalDate) {
  const deleting = await Item_direct.updateMany(
    { suplierID: suplierId, lastDate: { $ne: finalDate } },
    { amount: 0 }
  )
}

async function createItemWhole(articleArr, brandArr, title, error_path) {
  try {
    const searchImage = await searchImageInAllSupliers(
      articleArr,
      brandArr,
      error_path
    )
    ...
    const createdLink = await createOnlyLinkInBM(
    ...
    writeLogs(
      error_path,
      `createItemWhole  ${articleArr} ${brandArr} ${new Date()} - ${
        error.message
      }`
    )
  }
...
async function createItemDirect(
  ...
  try {
    ...
    writeLogs(
      error_path,
      `createItemDirectNewBM  ${articleArr} ${brandArr} ${new Date()} - ${
        error.message
      }`
    )
  }
...
module.exports = {
  ...
  getUuid,
  createOnlyLinkInBM,
  getFirstImageForItemFromBM,
  getCategoryByUuid,
  devideOnChunks,
  updateAllEndedProducts,
  createItemWhole,
  createItemDirect,
}
