...
async function updateMyPriceTmNewAll (array) {
    ...
    try{
        ...
        for (const onePackage of chunks) {
            ...
            const updating = await updateOneChunkItems(onePackage, suplierId, finalDate)
...
async function updateOneChunkItems(onePackage, suplierId, finalDate) {
    ...
    try{
        const promises = onePackage.map(item => updateOneItem(item, suplierId, finalDate))
        ...
        try {
             ...
             if (updating === null) {
                ...
                if (!isItemCreated) {
                    ...
                    const newItemWhole = await createItemWhole(articleArr, brandArr, titleArr, error_path)
