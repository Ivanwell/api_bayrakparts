const { Item_whole, Item_direct } = require('../../models/models.js')
const {
  getDate,
  writeLogs,
  devideOnChunks,
  ...
  createItemWhole,
  createItemDirect,
} = require('../common_functions/functions.js')
const {
  getJsonTechnoMirPrice,
} = require('../common_functions/get_prices_from_supliers.js')
...
async function mainTM() {
  try {
    const jsonPrice = await getJsonTechnoMirPrice()
    writeLogs(
      result_path,
      `${new Date()} - починаємо оновлення ТехноМир ${jsonPrice.data.length} шт`
    )
    updateMyPriceTmNewAll(jsonPrice.data)
    return 'Started'
  } catch (error) {
...
async function updateMyPriceTmNewAll(array) {
  try {
    ...
    for (const onePackage of chunks) {
      const updating = await updateOneChunkItems(
        onePackage,
        suplierId,
        finalDate
      )
    ...
    writeLogs(
      result_path,
      `${new Date()} Оновлено ТехноМир. Результати : оновлено : ${updatedItems}, створено нових : ${newItems}, створено локальних : ${newDirect}`
    )
  } catch (error) {
    writeLogs(
      error_path,
      `updateMyPriceTmNewAll  ${new Date()} - ${error.message}`
    )
  }
}

async function updateOneChunkItems(onePackage, suplierId, finalDate) {
  try {
    const promises = onePackage.map(item =>
      updateOneItem(item, suplierId, finalDate)
    )
    ...
    writeLogs(
      error_path,
      `updateOneChunkItems ${new Date()} - ${error.message}`
    )
  }
}

async function updateOneItem(item, suplierId, finalDate) {
  const articleArr = item.code.replace(/[- ./]/g, '').toUpperCase()
  const brandArr = item.brand
  const amountArr = item.quantity
  const priceArr = Math.ceil(+item.price.replace(',', '') * 40 * 1.2 + 80)
  ...
  try {
    const updating = await Item_direct.findOneAndUpdate(filter, update, {
      new: true,
    })
    ...
    if (updating === null) {
      ...
      if (!isItemCreated) {
        const newItemWhole = await createItemWhole(
          articleArr,
          brandArr,
          titleArr,
          error_path
        ...
        const newItemDirect = await createItemDirect(
          articleArr,
          brandArr,
          suplierId,
          finalDate,
    ...
    writeLogs(
      error_path,
      `${new Date()} ${articleArr} ${brandArr}- ${error.message}`
    )
  }
}
